...
// --- LÓGICA DE MODIFICAÇÃO (APENAS ADMIN/SUPERADMIN) ---
...
if (!$isSuperAdmin && !empty($user_forca_seguranca)) {
    $sql_documentos .= " AND (da.forca_seguranca IS NULL OR da.forca_seguranca = ?)";
...
<div class="main-content">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        ...
        <a href="checklist.php" style="text-decoration: none; color: #555;"><i class="fas fa-arrow-left"></i> Voltar</a>
    </div>
        ...
        <form action="documentos_aeronave.php?id=<?php echo $aeronave_id; ?>" method="POST" enctype="multipart/form-data">
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="nome_documento" name="nome_documento" placeholder="Ex: Manual de Voo" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <?php else: ?>
                        ...
                        <small>O documento será associado à sua força.</small>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    ...
                    <div class="radio-group">
                        <label class="radio-label"><input type="radio" name="associar_a" value="todos_modelos"> <div class="radio-text"><span>Associar a <strong>TODOS OS MODELOS</strong> de aeronaves</span><small>Ideal para documentos universais, como regulamentos e normas gerais.</small></div></label>
                        <label class="radio-label"><input type="radio" name="associar_a" value="modelo" checked> <div class="radio-text"><span>Associar a <strong>TODAS</strong> as aeronaves do modelo "<?php echo htmlspecialchars($modelo_aeronave_atual); ?>"</span><small>Ideal para manuais e documentos gerais deste modelo.</small></div></label>
                        <label class="radio-label"><input type="radio" name="associar_a" value="aeronave"> <div class="radio-text"><span>Associar <strong>SOMENTE</strong> a esta aeronave ("<?php echo htmlspecialchars($aeronave_details['prefixo']); ?>")</span><small>Ideal para apólices de seguro, registros específicos, etc.</small></div></label>
                    </div>
                </div>
            </div>
    ...
    <div class="table-container" style="margin-top: 30px;">
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php if (!empty($documentos_encontrados)): ?>
                    <?php foreach ($documentos_encontrados as $doc): ?>
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($doc['forca_associada'])): ?>
                                    <span class="badge forca" style="background-color: #333;"><?php echo htmlspecialchars($doc['forca_associada']); ?></span>
                                <?php else: ?>
                                    <span class="badge geral"><i class="fas fa-globe"></i> Geral</span>
                                <?php endif; ?>
                            </td>
                            <td class="action-buttons">
                                <a href="uploads/<?php echo rawurlencode($doc['nome_arquivo_servidor']); ?>" class="action-btn view-btn" target="_blank"><i class="fas fa-eye"></i> Visualizar</a>
                                <a href="uploads/<?php echo rawurlencode($doc['nome_arquivo_servidor']); ?>" class="action-btn download-btn" download><i class="fas fa-download"></i> Baixar</a>
                                <?php if ($isAdmin || $isSuperAdmin): ?>
                                    <a href="documentos_aeronave.php?id=<?php echo $aeronave_id; ?>&disassociate_id=<?php echo $doc['id']; ?>" class="action-btn disassociate-btn" onclick="return confirm('Atenção: A associação será removida. Continuar?');"><i class="fas fa-unlink"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="4">Nenhum documento encontrado para esta aeronave ou seu modelo.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
.badge.geral { background-color: #28a745; }
